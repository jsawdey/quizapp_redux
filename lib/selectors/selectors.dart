...
List<String> categoriesSelector(AppState state) {
  Set<String> cats = Set.from(state.questions.map((q) => q.category.toUpperCase()));
  return cats.toList();
}
...
List<Question> currCatQuestionsSelector(AppState state) =>
    ...
    state.currentCategory.toUpperCase()).toList();